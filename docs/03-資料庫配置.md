...
## 🔄 資料庫更新（EF Scaffold 一鍵重生）

> 依循 AI_CONTEXT 原則：把對外指令寫在文件中，供日後「無腦」重生 Domain/DbContext。
...
**用途**：覆寫並重新產生 DbContext（Infrastructure/Persistence/Contexts）與實體（Domain/Entities），保持現有專案結構與命名空間。
...
**用途**：同上，適用於命令列/CI/CD。`--force` 會覆寫現有檔案，執行前請先備份或確認無手改的模型程式碼。
...
2. 或使用環境變數設定連接字串（見上方「連接字串設定」）

#### ⚠️ 注意事項

- `--force` / `-Force` 會**覆寫**現有檔案，執行前請確認：
  - 已備份手動修改的實體類別
  - 或使用版本控制（Git）追蹤變更
- 若僅需特定資料表，可在指令後加上：
