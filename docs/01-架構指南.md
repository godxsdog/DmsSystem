...
- 介面：`IDividendService`（定義 `ImportAsync` / `ConfirmAsync`）
...
- DTO：
  - `DividendImportResult`：匯入結果（Success/Inserted/Updated/Failed/Errors）
  - `DividendConfirmResult`：計算結果（Nav、Unit、DivTot、DivRate、DivRateM、DivObj、DivObjAmt、DivPre…）
...
### 3. API 入口與呼叫鏈
1) 匯入 CSV（前端 / Postman）
   - HTTP：`POST /api/Dividends/import`（`multipart/form-data`，欄位 `file`）
   - Controller：`DividendsController.Import()` → `_service.ImportAsync(file)`
   - Service：`DividendService.ImportAsync`
     1. 讀檔（Big5），跳過前 3 行說明
     2. CsvHelper 解析 → `DividendCsvRow`（對應 `DividendCsvMap`）
     3. 逐筆呼叫 `DividendSqlQueries.UpsertFundDiv`（Dapper）寫入 `MDS.FUND_DIV`
     4. 回傳 `DividendImportResult`（新增/更新/失敗統計與錯誤訊息）

2) 配息計算與確認
   - HTTP：`POST /api/Dividends/{fundNo}/{dividendDate}/{dividendType}/confirm`
     - 例：`/api/Dividends/D109/2020-07-06/M`
   - Controller：`DividendsController.Confirm()` → `_service.ConfirmAsync(...)`
   - Service：`DividendService.ConfirmAsync`
     1. 開啟連線 + Transaction
     2. 取 NAV、Unit（`GetNav`、`GetUnit`）；缺資料即回傳錯誤
     3. 取當期 FUND_DIV（`GetFundDiv`）與分攤參數（`GetFundDivSet`）
     4. 取目標配息率（`GetFundDivObj`，回溯最近一筆）與上期配息率（`GetPreviousFundDiv`）
     5. 計算：
        - `DIV_TOT`（所有 pre/div 合計扣除費用）
        - `DIV_RATE`（每單位收益），年化配息率（依 M/Q/S/Y 換算）
        - 目標配息率 / 每單位配息金額（含回推）
        - 本金補足（`PreDiv1B`/`Div1B`）
     6. 更新 `MDS.FUND_DIV`（`UpdateFundDivCalculation`），Commit Transaction
     7. 回傳 `DividendConfirmResult`

3) 配息資料查詢
   - HTTP：`GET /api/Dividends?fundNo=&dividendType=&startDate=&endDate=`
   - Controller：`DividendsController.GetDividends()` → `IFundDivRepository.GetAllAsync`
   - Repository：`FundDivRepository`（EF Core）從 `MDS.FUND_DIV` 讀取並回傳集合

### 4. 前端對應（react-client/src/pages/Dividend.tsx）
- 匯入：`fetch POST /api/Dividends/import` 上傳 CSV，顯示 `DividendImportResult`
- 確認：`fetch POST /api/Dividends/{fundNo}/{date}/{type}/confirm`，顯示 `DividendConfirmResult`
- 查詢：`fetch GET /api/Dividends`，依條件篩選並呈現表格

### 5. 資料流總結
Controller → `IDividendService` → `DividendService` → `IDbConnectionFactory` → Dapper SQL (`DividendSqlQueries`) → DB  
回傳 DTO (`DividendImportResult` / `DividendConfirmResult`) → Controller → JSON → 前端
